document.addEventListener("DOMContentLoaded", async () => {
  ...
  const navigationType =
    navigationEntries.length > 0 ? navigationEntries[0].type : "";
  ...
  const reservationsSection = document.getElementById(
  ...
  async function loadRooms() {
    ...
    try {
      ...
      renderError(
        roomListContainer,
        error.message || "객실 목록을 불러오지 못했습니다.",
      );
    }
  }
  ...
  async function handleRoomActivationChange(input) {
    ...
    const room_name = targetRoom?.room_name || `Room ID ${roomId}`;
    const confirmationMessage = nextValue
      ? `"${room_name}" 객실을 활성화하시겠습니까?`
      : `"${room_name}" 객실을 비활성화하시겠습니까?`;
  ...
  function bindPageVisibilityEvents() {
    window.addEventListener("pageshow", async (event) => {
      ...
      const pageNavigationType =
        pageNavigationEntries.length > 0 ? pageNavigationEntries[0].type : "";
